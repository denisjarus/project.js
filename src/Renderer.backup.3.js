function Renderer(context) {
    ...
    function cacheGeometry(geometry) {
        if (geometryCache[geometry.id] === undefined) {
            ...
            bufferData(gl.ELEMENT_ARRAY_BUFFER, getIndexBuffer(geometry), geometry.indices);

            for (var attribute, i = 0; attribute = geometry.attributes[i]; i++) {
                bufferData(gl.ARRAY_BUFFER, getVertexBuffer(geometry, attribute), geometry.getData(attribute));
            }
    ...
    function getVertexBuffer(geometry, attribute) {
        var glBuffer = geometryCache[geometry.id].vertexBuffers[attribute];

        if (glBuffer === undefined) {
            glBuffer = geometryCache[geometry.id].vertexBuffers[attribute] = gl.createBuffer();
        }
    ...
    function onVerticesUpdate(event) {
        bufferData(gl.ARRAY_BUFFER, getVertexBuffer(event.target, event.attribute), event.data);
    }
